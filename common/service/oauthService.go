...
func GetGithubToken(code string) (string, common.GFError) {
	...
	// 请求
	respDataStr, httpErr := util.GetByHttpWithParams(url, headersMap, paramsMap, 10000*time.Millisecond)
	if httpErr != nil {
		log.Warn(httpErr)
...
func GetGiteeToken(code string) (string, common.GFError) {
	...
	// 请求
	respDataStr, httpErr := util.PostByHttpWithParams(url, headersMap, paramsMap, 10000*time.Millisecond)
	if httpErr != nil {
		log.Warn(httpErr)
		return "", common.NewServiceError(httpErr.Error())
	}
